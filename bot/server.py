...
Routes:
  GET /dl/{token}   — download (God Speed from disk OR dual Telegram stream)
...
from bot.stream import iter_file, iter_cached_file, cache_path, parse_range_header
...
def _is_rate_limited(ip: str) -> bool:
    ...
    if len(_rate[ip]) >= Config.RATE_LIMIT_PER_MINUTE:
        return True
    _rate[ip].append(now)
    return False
...
async def handle_download(request: web.Request) -> web.StreamResponse:
    ...
    range_hdr    = request.headers.get("Range") if known_size else None
    start, end   = parse_range_header(range_hdr, file_size) if known_size else (0, 0)
    ...
    if range_hdr and known_size:
        headers["Content-Range"] = f"bytes {start}-{end}/{file_size}"

    status = 206 if (range_hdr and known_size) else 200
    ...
    try:
        ...
        if god_speed_ready and cached.exists():
            ...
            c1 = pool.get()
            async for chunk in iter_file(c1, message_id, Config.CHANNEL_ID, offset=start):
                if known_size and bytes_sent >= content_len:
                    break
                if known_size:
                    rem = content_len - bytes_sent
...
async def handle_info(request: web.Request) -> web.Response:
    token = request.match_info["token"]
    fm = await cache.cache_get(token)
    if fm is not None:
        ea = fm.get("expires_at")
        if ea and int(time.time()) > ea:
            await cache.cache_delete(token)
            fm = None
    if fm is None:
        fm = await database.get_file(token)
        if fm is None:
            raise web.HTTPNotFound(text="File not found.")
        await cache.cache_set(token, fm)
    return web.json_response({
        ...
        "god_speed":  fm.get("god_speed_ready", False),
    })
